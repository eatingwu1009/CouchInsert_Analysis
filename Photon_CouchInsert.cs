...
namespace VMS.TPS
{
    public class Script
    {
        ...
        public void Execute(ScriptContext scriptContext, ScriptEnvironment environment)
        {
            ...
            Structure OriCouchSurface = SS.Structures.FirstOrDefault(e => e.Id == "Ori_CouchSurface");
            Structure OriCouchInterior = SS.Structures.FirstOrDefault(e => e.Id == "Ori_CouchInterior");
            foreach (VVector[] vectors in OriCouchSurface.GetContoursOnImagePlane(1))
            {
                foreach (VVector v in vectors)
                {
                    double x = v.x;
            ...
            double MMMX = MaxMinDetect(CSVVector, orientation)[0]; double MMMY = MaxMinDetect(CSVVector, orientation)[1];
            ...
            if (SS.CanAddCouchStructures(out errorCouch) == true)
            {
                ...
                double ShiftX = MMMX - MMX;
                double ShiftY = (MMMY) - MMY;
